...
-- Write your SQL solution here
SELECT 
    O.OrderID, C.CustomerName, E.FirstName, E.LastName
FROM
    orders O
        JOIN
...
SELECT 
    c.CategoryName, p.ProductName, AVG(Price)
FROM
    products p
        JOIN
...
SELECT 
    O.EmployeeID, COUNT(OrderId) AS OrderCount
FROM
    orders o
        JOIN
    employees e ON o.employeeId = e.employeeid
...
with Spendings as (Select c.customerid,c.CustomerName, d.quantity*p.price as Spent
From Orderdetails d
...
SELECT 
    CustomerName, SUM(Spent) AS TotalSpent
FROM
    Spendings
GROUP BY Customerid
...
FROM
    (SELECT 
        OrderID, (d.quantity * p.price) AS OrderValue
    FROM
        orderdetails d
    JOIN products p ON d.productId = p.productid
    GROUP BY OrderId , OrderValue
    HAVING OrderValue >= 2000) AS OderValue;
...
SELECT 
    c.CustomerName,
    d.OrderID,
    d.quantity * p.price AS TotalValue
FROM
    Orderdetails d
        JOIN
...
GROUP BY c.CustomerName , d.OrderID , TotalValue
HAVING TotalValue = (SELECT 
        MAX(d.quantity * p.price) AS TotalValue
    FROM
        Orderdetails d
            JOIN
        products p ON d.productId = p.productid);
